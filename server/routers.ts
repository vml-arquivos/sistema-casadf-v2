...
const leadsRouter = router({
  ...
  create: publicProcedure
    .input(z.object({
      ...
      message: z.string().optional(), // Campo para mensagem inicial
    }))
    .mutation(async ({ input }) => {
      // 1. Qualificação do Lead por IA
      const message = input.notes || input.message || "Lead sem mensagem inicial. Qualificação baseada apenas na fonte.";
      const source = input.source || "site";

      const aiQualification = await qualifyLead({
        message,
        source: source as any,
        interestedPropertyId: input.interestedPropertyId,
      });

      // 2. Criação do Lead com dados da IA
      return await db.createLead({
        ...input,
        stage: "novo",
        qualification: aiQualification.qualification,
        buyerProfile: aiQualification.buyerProfile,
        urgencyLevel: aiQualification.urgencyLevel,
        transactionInterest: aiQualification.transactionInterest,
        budgetMin: aiQualification.budgetMin,
        budgetMax: aiQualification.budgetMax,
        preferredNeighborhoods: aiQualification.preferredNeighborhoods,
        preferredPropertyTypes: aiQualification.preferredPropertyTypes,
        notes: aiQualification.notes,
      });
    }),
...
const integrationRouter = router({
  ...
  whatsappWebhook: publicProcedure
    ...
    .mutation(async ({ input }) => {
      ...
      // Se for uma mensagem de entrada, acionar o Agente de IA
      if (input.type === "incoming") {
        const { processIncomingMessage } = await import("./services/aiAgent");
        aiResponse = await processIncomingMessage(input.phone, input.content);
        
        // TODO: Enviar aiResponse de volta para o WhatsApp (via N8N ou outro serviço)
        // Por enquanto, apenas logamos a resposta
        console.log(`Resposta da IA para ${input.phone}: ${aiResponse}`);
      }
      ...
      await db.createWebhookLog({
        ...
        response: JSON.stringify({ success: true, messageId: message.id, aiResponse }),
        status: "success",
      });
      
      return { success: true, messageId: message.id, aiResponse };
    }),
